	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try(PrintWriter printWriter=response.getWriter()){
			...
			if(admin!=null) {
				...
				response.sendRedirect("admin/ViewCitizens");
				...
				response.sendRedirect("citizen/ViewCitizenComplaint");
				System.out.println(session.getAttribute("user"));
			}else if(dept!=null){
				session.setAttribute("user", dept);
				session.setAttribute("name", dept.getDeptHead());
				session.setAttribute("dept_name", dept.getDeptName());
				session.setAttribute("role", "dept");
				response.sendRedirect("dept/ViewComplaint");
				System.out.println(session.getAttribute("user"));
			}else {
