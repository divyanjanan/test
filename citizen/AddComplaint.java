 ...
 */
@WebServlet("/citizen/RegComplaint")
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		Citizens user = (Citizens) session.getAttribute("user");
			try(PrintWriter printWriter=response.getWriter()){
				...
				String msg = service.addComplaint(complaint);
				//response.sendRedirect("ViewCitizenComplaint");
				
				PrintWriter out = response.getWriter();
				out.println("<script type=\"text/javascript\">");
				   out.println("alert('"+msg+"');");
				   out.println("location='ViewCitizenComplaint';");
				   out.println("</script>");
